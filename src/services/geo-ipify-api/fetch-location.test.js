...
import 'whatwg-fetch';
import { server, testData, ERROR_API_KEY, ERROR_INVALID_IP } from 'test-mocks/geo-ipify-api';
import { composeResult } from './compose-result';

beforeAll(() => {
  server.listen();
  process.env.GEO_IPIFY_API_KEY = 'TEST_API_KEY';
});
...
describe('fetchLocation()', () => {
  ...
  it('should return error object when no API key is provided', async () => {
    ...
    expect(result).toEqual({
      ...
      payload: { errorMessage: ERROR_API_KEY.messages },
    });
  });
  ...
  it('should return error object when invalid IP is provided', async () => {
    ...
    expect(result).toEqual({
      ...
      payload: { errorMessage: ERROR_INVALID_IP.messages },
    });
  });
});
